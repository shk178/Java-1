...
public class Session implements Runnable {
    ...
    public void run() {
        ...
        // 클라이언트로 데이터를 보내기 위한 출력 스트림
        DataOutputStream output = null;
        // 서버 콘솔에서 입력을 받기 위한 Scanner 객체
        Scanner sc = null;
        ...
        try {
            ...
            // 클라이언트로부터 메시지를 받는 별도의 스레드 생성
            Thread receiver = new Thread(() -> {
                try {
                    // 스레드가 중단되지 않는 동안 계속 실행
                    while (!Thread.currentThread().isInterrupted()) {
                        // 클라이언트로부터 UTF 형식의 문자열 읽기
                        String msg = finalInput.readUTF();
                        // 받은 메시지가 "종료"인지 확인
                        if (msg.equals("종료")) {
                            // 클라이언트가 연결을 종료했음을 로그에 출력
                            log("클라이언트가 연결 종료함");
                            // 반복문 탈출
                            break;
                        }
            ...
            // 서버 콘솔에서 입력을 받기 위한 Scanner 객체 생성
            sc = new Scanner(System.in);
            // 무한 루프: 서버 관리자가 "종료"를 입력할 때까지 계속
            while (true) {
                // 서버 관리자에게 입력 안내 메시지 출력
                log("보낼 내용:");
                // 서버 관리자로부터 한 줄 입력 받기
